...
<body>
  ...
  if ($cat != 0) {
    $sql = "SELECT * FROM `product_cat` where id=$cat";
  } else {
    $sql = "SELECT * FROM `product_cat`";
  }
  ...
  if ($result = $mysqli->query($sql)) {
    while ($row = $result->fetch_row()) {
  ?>
      <div class="container-fluid productdiv mt-5 mb-5">
        <p class="textindex"><?php echo $row[1]; ?></p>
        <div class="row">

          <?php

          $sqlProduct = "SELECT p.id, p.name, p.size, p.price, p.material, p.color, p.picture FROM `products` as p where p.id_cat=$row[0]";
          if ($resultProduct = $mysqli->query($sqlProduct)) {
            while ($Product = $resultProduct->fetch_row()) {
          ?>
              <div class="img-fluid col-sm-12 col-md-6 col-lg-3 mb-4 mt-4">

                <div class=" containerpro">
                  <img src="upload/<?php echo $Product[6]; ?>" alt="Avatar" class="gallery">
                  <div class="overlay">
                    <a href="favorite.php"><img src="icons/heart.png" class="iconh"> </a>
                    <a href="product.php"><img src="icons/shop.png" class="iconsh"></a>
                    <a target="_blank" href="./upload/<?php echo $Product[6]; ?>"><img src="icons/zoom.png" class="iconz"></a>
                  </div>
                </div>

                <div class="protext">
                  <h5> دسته بندی :<?php echo $row[1]; ?></h5>
                  نام محصول :<?php echo $Product[1]; ?><br>
                  جنس : <?php echo $Product[4]; ?> /
                  رنگ : <?php echo $Product[5]; ?> <br>
                  سایز : <?php echo $Product[2]; ?> /
                  قیمت :<?php echo $Product[3]; ?>
                </div>
              </div>
          <?php
            }
